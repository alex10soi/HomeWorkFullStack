...
// The name of the table in the MySQL database for storing data from the chat form
define('TABLE_NAME', 'chatInfo');
...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
	...
	if($file){
	  ...
	  $sec = strtotime($currentTime);
		...
		$sql_requst = "INSERT INTO " . TABLE_NAME . " (`chat_username`, `time`, `sec`, `message`) VALUES ('$key', '$currentTime', '$sec', '$message');";
		...
		// information about time, chat_username, message and write to message.json file and write all to DATABASE MySQL
		if($file[0]->$key){
			$newObjMessage = getNewObjMessage($message, $currentTime, $key, $sec);
			array_push($file[0]->$key, $newObjMessage);
			saveFile ($file); 

			saveInfoToDB('', $sql_requst);
